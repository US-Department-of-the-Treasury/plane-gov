...
export class WebhookStoreLegacy implements IWebhookStore {
  ...
  get webhooks() {
    return useWebhookStore.getState().webhooks;
  }

  get webhookSecretKey() {
    return useWebhookStore.getState().webhookSecretKey;
  }
  ...
  getWebhookById = (webhookId: string) => {
    return useWebhookStore.getState().getWebhookById(webhookId);
  };

  fetchWebhooks = async (workspaceSlug: string) => {
    return useWebhookStore.getState().fetchWebhooks(workspaceSlug);
  };

  fetchWebhookById = async (workspaceSlug: string, webhookId: string) => {
    return useWebhookStore.getState().fetchWebhookById(workspaceSlug, webhookId);
  };

  createWebhook = async (workspaceSlug: string, data: Partial<IWebhook>) => {
    return useWebhookStore.getState().createWebhook(workspaceSlug, data);
  };

  updateWebhook = async (workspaceSlug: string, webhookId: string, data: Partial<IWebhook>) => {
    return useWebhookStore.getState().updateWebhook(workspaceSlug, webhookId, data);
  };

  removeWebhook = async (workspaceSlug: string, webhookId: string) => {
    return useWebhookStore.getState().removeWebhook(workspaceSlug, webhookId);
  };

  regenerateSecretKey = async (workspaceSlug: string, webhookId: string) => {
    return useWebhookStore.getState().regenerateSecretKey(workspaceSlug, webhookId);
  };

  clearSecretKey = () => {
    useWebhookStore.getState().clearSecretKey();
  };
}

// Export types for backwards compatibility
export type { WebhookStoreState, WebhookStoreActions };
