 ...
 * @deprecated Use useWorkspaceLinkStore hook directly in React components
 */
export class WorkspaceLinkStoreLegacy implements IWorkspaceLinkStore {
  constructor() {
    // No-op constructor for compatibility
  }

  get links() {
    return useWorkspaceLinkStore.getState().links;
  }

  get linkMap() {
    return useWorkspaceLinkStore.getState().linkMap;
  }

  get linkData() {
    return useWorkspaceLinkStore.getState().linkData;
  }

  get isLinkModalOpen() {
    return useWorkspaceLinkStore.getState().isLinkModalOpen;
  }

  getLinksByWorkspaceId = (workspaceSlug: string) => {
    return useWorkspaceLinkStore.getState().getLinksByWorkspaceId(workspaceSlug);
  };

  getLinkById = (linkId: string) => {
    return useWorkspaceLinkStore.getState().getLinkById(linkId);
  };

  setLinkData = (link: TLink | undefined) => {
    return useWorkspaceLinkStore.getState().setLinkData(link);
  };

  toggleLinkModal = (isOpen: boolean) => {
    return useWorkspaceLinkStore.getState().toggleLinkModal(isOpen);
  };

  addLinks = (workspaceSlug: string, links: TLink[]) => {
    return useWorkspaceLinkStore.getState().addLinks(workspaceSlug, links);
  };

  fetchLinks = (workspaceSlug: string) => {
    return useWorkspaceLinkStore.getState().fetchLinks(workspaceSlug);
  };

  createLink = (workspaceSlug: string, data: Partial<TLink>) => {
    return useWorkspaceLinkStore.getState().createLink(workspaceSlug, data);
  };

  updateLink = (workspaceSlug: string, linkId: string, data: Partial<TLink>) => {
    return useWorkspaceLinkStore.getState().updateLink(workspaceSlug, linkId, data);
  };

  removeLink = (workspaceSlug: string, linkId: string) => {
    return useWorkspaceLinkStore.getState().removeLink(workspaceSlug, linkId);
  };
}
