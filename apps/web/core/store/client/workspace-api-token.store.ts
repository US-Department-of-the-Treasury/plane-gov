 ...
 * @deprecated Use useWorkspaceApiTokenStore hook directly in React components
 */
export class ApiTokenStoreLegacy implements IApiTokenStore {
  ...
  get apiTokens() {
    return useWorkspaceApiTokenStore.getState().apiTokens;
  }

  getApiTokenById = (apiTokenId: string) => {
    return useWorkspaceApiTokenStore.getState().getApiTokenById(apiTokenId);
  };

  fetchApiTokens = () => {
    return useWorkspaceApiTokenStore.getState().fetchApiTokens();
  };

  fetchApiTokenDetails = (tokenId: string) => {
    return useWorkspaceApiTokenStore.getState().fetchApiTokenDetails(tokenId);
  };

  createApiToken = (data: Partial<IApiToken>) => {
    return useWorkspaceApiTokenStore.getState().createApiToken(data);
  };

  deleteApiToken = (tokenId: string) => {
    return useWorkspaceApiTokenStore.getState().deleteApiToken(tokenId);
  };
}
