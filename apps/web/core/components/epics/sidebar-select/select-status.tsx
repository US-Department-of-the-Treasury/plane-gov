...
type Props = {
  control: Control<Partial<IEpic>, any>;
  submitChanges: (formData: Partial<IEpic>) => void;
  watch: UseFormWatch<Partial<IEpic>>;
};
              ...
              label={
                <span className={`flex items-center gap-2 text-left capitalize ${value ? "" : "text-primary"}`}>
                  <span
                    ...
                    style={{
                      backgroundColor: EPIC_STATUS?.find((option) => option.value === value)?.color,
                    }}
                  />
                  {watch("status")}
                </span>
