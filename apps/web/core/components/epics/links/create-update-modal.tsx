...
type Props = {
  createLink: (formData: EpicLink) => Promise<void>;
  data?: ILinkDetails | null;
  isOpen: boolean;
  handleClose: () => void;
  updateLink: (formData: EpicLink, linkId: string) => Promise<void>;
};

const defaultValues: EpicLink = {
  title: "",
  url: "",
};
...
export function CreateUpdateModuleLinkModal(props: Props) {
  ...
  const handleFormSubmit = async (formData: EpicLink) => {
