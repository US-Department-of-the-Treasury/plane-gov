...
import { History, RefreshCw, X, ChevronRight, Clock, User } from "lucide-react";
// plane imports
...
// types
import type { TWikiPageVersion, TWikiPageVersionDetail } from "@plane/types";
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isSelected
          ? "bg-custom-primary-100/10 border border-custom-primary-100/30"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-custom-text-400 mt-0.5">
          {new Date(version.last_saved_at).toLocaleString()}
        </p>
      </div>
      <ChevronRight
        className={cn(
          "size-4 text-custom-text-400 flex-shrink-0 transition-transform",
...
const VersionPreview = memo(function VersionPreview({
  ...
  const { data: version, isLoading } = useWikiPageVersionDetails(
    workspaceSlug,
    pageId,
    versionId
  );
  ...
  if (!version) {
    return (
      <div className="flex items-center justify-center h-64 text-custom-text-400">
        Version not found
      </div>
    );
  }
        ...
        <div className="prose prose-sm max-w-none">
          {version.description_html ? (
            <div
              ...
              dangerouslySetInnerHTML={{ __html: version.description_html }}
            />
          ) : (
...
export const WikiVersionHistoryPanel = memo(function WikiVersionHistoryPanel({
  workspaceSlug,
  pageId,
  isOpen,
  onClose,
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
  } = useWikiPageVersions(workspaceSlug, pageId);

  const restoreVersionMutation = useRestoreWikiPageVersion();
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex items-center justify-between px-4 py-3 border-b border-custom-border-200">
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-1 rounded hover:bg-custom-background-80"
            >
              <X className="size-4" />
            </button>
          </div>
            ...
            {selectedVersionId ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center px-4">
                ...
                <p className="text-sm text-custom-text-400 mt-1">
                  You can compare and restore previous versions of this page
                </p>
              </div>
            )}
          </div>
