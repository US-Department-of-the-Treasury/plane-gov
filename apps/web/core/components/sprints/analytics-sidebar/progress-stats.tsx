...
import type { TSprintDistribution, TSprintEstimateDistribution, TSprintPlotType } from "@plane/types";
...
type TSprintProgressStats = {
  sprintId: string;
  distribution: TSprintDistribution | TSprintEstimateDistribution | undefined;
  groupedIssues: Record<string, number>;
  handleFiltersUpdate: (condition: TWorkItemFilterCondition) => void;
  isEditable?: boolean;
  noBackground?: boolean;
  plotType: TSprintPlotType;
  roundedTab?: boolean;
...
export function SprintProgressStats(props: TSprintProgressStats) {
  ...
  // store imports
  const { storedValue: currentTab, setValue: setSprintTab } = useLocalStorage(
    `sprint-analytics-tab-${sprintId}`,
    "stat-assignees"
  );
  // derived values
  const currentTabIndex = (tab: string): number => PROGRESS_STATS.findIndex((stat) => stat.key === tab);
  const currentDistribution = distribution as TSprintDistribution;
  const currentEstimateDistribution = distribution as TSprintEstimateDistribution;
