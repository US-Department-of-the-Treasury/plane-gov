...
// assets
import darkAssigneeAsset from "@/app/assets/empty-state/active-sprint/assignee-dark.webp?url";
import lightAssigneeAsset from "@/app/assets/empty-state/active-sprint/assignee-light.webp?url";
import darkLabelAsset from "@/app/assets/empty-state/active-sprint/label-dark.webp?url";
import lightLabelAsset from "@/app/assets/empty-state/active-sprint/label-light.webp?url";
import darkPriorityAsset from "@/app/assets/empty-state/active-sprint/priority-dark.webp?url";
import lightPriorityAsset from "@/app/assets/empty-state/active-sprint/priority-light.webp?url";
...
export type ActiveSprintStatsProps = {
...
export function ActiveSprintStats(props: ActiveSprintStatsProps) {
  ...
  }, [workspaceSlug, projectId, sprintId, issuesLoaderElement, sprintIssueDetails?.nextPageResults]);
    ...
    <div className="flex flex-col gap-4 p-4 min-h-[17rem] overflow-hidden bg-surface-1 col-span-1 lg:col-span-2 xl:col-span-1 border border-subtle rounded-lg">
      ...
      >
        ...
        >
          ...
          >
            {t("project_sprints.active_sprint.priority_issue")}
          </Tab>
          <Tab
            className={({ selected }) =>
              cn(
          ...
          >
            {t("project_sprints.active_sprint.assignees")}
          </Tab>
          <Tab
            className={({ selected }) =>
              cn(
          ...
          >
            {t("project_sprints.active_sprint.labels")}
          </Tab>
        </Tab.List>
        ...
        <Tab.Panels as={Fragment}>
          ...
          >
            ...
            >
              {sprintIssueDetails && "issueIds" in sprintIssueDetails ? (
                sprintIssueDetails.issueCount > 0 ? (
                  <>
                    {sprintIssueDetails.issueIds.map((issueId: string) => {
                    ...
                    {(sprintIssueDetails.nextPageResults === undefined || sprintIssueDetails.nextPageResults) && (
                      <div
                  ...
                  <div className="flex items-center justify-center h-full w-full">
                    <SimpleEmptyState
                      title={t("active_sprint.empty_state.priority_issue.title")}
          ...
          <Tab.Panel
            ...
            className="flex h-52 w-full flex-col gap-1 overflow-y-auto text-secondary vertical-scrollbar scrollbar-sm"
          >
            {sprint && !isEmpty(sprint.distribution) ? (
              sprint?.distribution?.assignees && sprint.distribution.assignees.length > 0 ? (
                sprint.distribution?.assignees?.map((assignee, index) => {
                ...
                <div className="flex items-center justify-center h-full w-full">
                  <SimpleEmptyState
                    title={t("active_sprint.empty_state.assignee.title")}
          ...
          >
            {sprint && !isEmpty(sprint.distribution) ? (
              sprint?.distribution?.labels && sprint.distribution.labels.length > 0 ? (
                sprint.distribution.labels?.map((label, index) => (
                  <SingleProgressStats
                ...
                <div className="flex items-center justify-center h-full w-full">
                  <SimpleEmptyState title={t("active_sprint.empty_state.label.title")} assetPath={labelsResolvedPath} />
                </div>
              )
            ) : (
              loaders
