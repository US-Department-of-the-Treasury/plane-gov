...
// types
import { EDocumentSharePermission } from "@plane/types";
import type { TDocumentShareFormData } from "@plane/types";
...
export function ShareDocumentModal({ workspaceSlug, pageId, isOpen, onClose }: ShareDocumentModalProps) {
  const [copiedLink, setCopiedLink] = useState(false);

  // Queries
  const { data: shares, isLoading: sharesLoading } = useDocumentShares(workspaceSlug, pageId);
  const { data: members } = useWorkspaceMembers(workspaceSlug);
  const createShareMutation = useCreateDocumentShare();
  const deleteShareMutation = useDeleteDocumentShare();
  ...
  const {
    ...
    defaultValues: {
      user: "",
      permission: EDocumentSharePermission.VIEW,
    },
  });
  ...
  const onSubmit = (data: FormData) => {
    const payload: TDocumentShareFormData = {
      user: data.user,
  ...
  const handleCopyLink = () => {
    const link = `${window.location.origin}/${workspaceSlug}/documents/${pageId}`;
        ...
        <div className="flex items-center gap-2 mb-6 p-3 bg-custom-background-80 rounded-lg">
          ...
          <span className="text-sm flex-1 truncate">
            {typeof window !== "undefined"
              ? `${window.location.origin}/${workspaceSlug}/documents/${pageId}`
              : `/${workspaceSlug}/documents/${pageId}`}
          </span>
          <Tooltip tooltipContent={copiedLink ? "Copied!" : "Copy link"}>
            <button type="button" className="p-1.5 rounded hover:bg-custom-background-90" onClick={handleCopyLink}>
              {copiedLink ? <Check className="size-4 text-green-500" /> : <Copy className="size-4" />}
            </button>
          </Tooltip>
        </div>

        {/* Add user form */}
        <form onSubmit={(e) => void handleSubmit(onSubmit)(e)} className="mb-6">
          <div className="flex items-end gap-2">
            <div className="flex-1">
              <label htmlFor="share-member-select" className="block text-sm font-medium mb-1">
                Add people
              </label>
              <Controller
                name="user"
              ...
              <Controller
                ...
                render={({ field }) => (
                  <select
                    ...
                    className="w-full px-3 py-2 text-sm border border-custom-border-200 rounded-md bg-custom-background-100 outline-none focus:border-custom-primary-100"
                  >
                    <option value={EDocumentSharePermission.VIEW}>Viewer</option>
                    <option value={EDocumentSharePermission.EDIT}>Editor</option>
                  </select>
                )}
              />
            </div>
        ...
        <div>
          ...
          {sharesLoading ? (
            ...
            <div className="space-y-2">
              {shares.map((share) => (
                <div key={share.id} className="flex items-center gap-3 p-2 rounded-md hover:bg-custom-background-80">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-custom-text-400 capitalize">
                      {share.permission === EDocumentSharePermission.EDIT ? "Editor" : "Viewer"}
                    </p>
                  </div>
                  <button
                    type="button"
