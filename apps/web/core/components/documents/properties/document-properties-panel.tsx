...
interface DocumentPropertiesPanelProps {
  ...
  pageId: string;
  page: TDocumentDetail;
  isEditable?: boolean;
}

export const DocumentPropertiesPanel = memo(function DocumentPropertiesPanel({
  workspaceSlug,
  pageId,
  page,
  isEditable = true,
  ...
  // Mutations
  const updatePageMutation = useUpdateDocument();
  const createPropertyMutation = useCreatePageProperty();
  const updatePropertyMutation = useUpdatePageProperty();

  // Filter property definitions based on document type
  const filteredDefinitions = useMemo(() => {
    if (!propertyDefinitions) return [];
    const documentType = page.document_type || "page";
    return propertyDefinitions.filter(
      (def) => def.document_types.length === 0 || def.document_types.includes(documentType)
    );
  }, [propertyDefinitions, page.document_type]);

  // Create a map of property definition id -> property value
  const propertyValueMap = useMemo(() => {
    const map = new Map<string, TDocumentPropertyValue>();
    if (pageProperties) {
  ...
  const handlePageTypeChange = useCallback(
    async (newType: TDocumentType) => {
      try {
        await updatePageMutation.mutateAsync({
          workspaceSlug,
          pageId,
          data: { document_type: newType },
        });
      } catch (error) {
        ...
        <div className={cn("mb-2 mt-4 space-y-2.5", { "opacity-60": !isEditable })}>
          ...
          <SidebarPropertyListItem icon={Type} label="Type">
            <DocumentTypeSelector
              value={(page.document_type as TDocumentType) || "page"}
          ...
          {filteredDefinitions.length === 0 && (
            <div className="py-4 text-center text-sm text-custom-text-400">
              No properties available for this document type.
            </div>
          )}
        </div>
      </div>
