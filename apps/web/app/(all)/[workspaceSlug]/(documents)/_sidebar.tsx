...
const DocumentPageItem = memo(function DocumentPageItem({
  page,
  workspaceSlug,
  activePageId,
  depth = 0,
    ...
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <div>
        <div
          ...
          onClick={() => router.push(`/${workspaceSlug}/documents/${page.id}`)}
          onKeyDown={(e) => {
            if (e.key === "Enter" || e.key === " ") {
              e.preventDefault();
              router.push(`/${workspaceSlug}/documents/${page.id}`);
            }
          }}
        >
 ...
 */
const DocumentHeaderActions = memo(function DocumentHeaderActions({ onCreatePage }: { onCreatePage: () => void }) {
  return (
...
/**
 * Document navigation items - matches Projects sidebar pattern with Home, Your work, Drafts
 */
const DocumentNavigationItems = memo(function DocumentNavigationItems({ workspaceSlug }: { workspaceSlug: string }) {
  ...
  const navItems = [
    {
      ...
      icon: HomeIcon,
      isActive: (path: string) => path === `/${workspaceSlug}/documents` || path === `/${workspaceSlug}/documents/`,
    },
    {
      key: "recent",
      label: "Recent",
      href: `/${workspaceSlug}/documents/recent`,
      icon: Clock,
      isActive: (path: string) => path.includes("/documents/recent"),
    },
    {
      key: "starred",
      label: "Starred",
      href: `/${workspaceSlug}/documents/starred`,
      icon: Star,
      isActive: (path: string) => path.includes("/documents/starred"),
    },
  ];
 ...
 */
const DocumentPagesSection = memo(function DocumentPagesSection({
  pages,
  collections,
  workspaceSlug,
  activePageId,
  ...
  pages: TDocumentTreeNode[];
  collections: TDocumentCollectionTreeNode[];
  workspaceSlug: string;
  activePageId?: string;
  onCreateChildPage: (parentId: string) => void;
  isLoading: boolean;
  ...
  const { storedValue: isPagesOpen, setValue: togglePagesOpen } = useLocalStorage<boolean>(
    "is_document_pages_open",
    true
  );
    ...
    <Collapsible open={!!isPagesOpen} onOpenChange={togglePagesOpen} className="flex flex-col">
      <div className="group w-full flex items-center justify-between px-2 py-1.5 rounded-sm text-placeholder hover:bg-layer-transparent-hover">
        <CollapsibleTrigger
          ...
          aria-label={isPagesOpen ? "Close documents menu" : "Open documents menu"}
        >
          <span className="text-13 font-semibold">Documents</span>
        </CollapsibleTrigger>
        <div className="flex items-center gap-1">
          <Tooltip tooltipHeading="Create document" tooltipContent="">
            <IconButton
              variant="ghost"
              size="sm"
              icon={Plus}
...
/**
 * Document mode sidebar with page tree and collections
...
export function DocumentSidebar() {
  ...
  // store hooks - using document-specific collapse state
  const {
  ...
  const { storedValue, setValue } = useLocalStorage("documentSidebarWidth", SIDEBAR_WIDTH);
  ...
  const handleCreateChildPage = useCallback(
    (parentId: string) => {
      router.push(`/${workspaceSlug}/documents?create=true&parent=${parentId}`);
    },
  ...
  return (
    <ResizableSidebar
      ...
      defaultCollapsed={documentsSidebarCollapsed}
      peekDuration={1500}
      onWidthChange={handleWidthChange}
      onCollapsedChange={toggleDocumentsSidebar}
      isCollapsed={documentsSidebarCollapsed}
      toggleCollapsed={toggleDocumentsSidebar}
    ...
    >
      <>
        <SidebarWrapper
          title="Documents"
          headerActions={<DocumentHeaderActions onCreatePage={handleCreatePage} />}
          onToggle={toggleDocumentsSidebar}
        >
          {/* Navigation items - matches Projects sidebar pattern */}
          <DocumentNavigationItems workspaceSlug={workspaceSlug || ""} />
