...
export class ProjectMemberFiltersStoreLegacy implements IProjectMemberFiltersStore {
  constructor() {
    // No initialization needed - Zustand store is already created
  }

  // Getter for filtersMap that delegates to Zustand store
  get filtersMap(): Record<string, IMemberFilters> {
    return useProjectMemberFiltersStore.getState().filtersMap;
  }
  ...
  getFilteredMemberIds = (
    ...
    return useProjectMemberFiltersStore.getState().getFilteredMemberIds(members, memberDetailsMap, getMemberKey, projectId);
  };
   ...
   */
  getFilters = (projectId: string): IMemberFilters | undefined => {
    return useProjectMemberFiltersStore.getState().getFilters(projectId);
  };
   ...
   */
  updateFilters = (projectId: string, filters: Partial<IMemberFilters>): void => {
    return useProjectMemberFiltersStore.getState().updateFilters(projectId, filters);
  };
}
