"""
Management command to migrate Issues to Documents.

This command migrates existing Issue records to Document records with document_type="issue".
...
Usage:
    python manage.py migrate_issues_to_documents
    python manage.py migrate_issues_to_documents --workspace=<slug>
    python manage.py migrate_issues_to_documents --dry-run
"""
...
# Map IssueRelation types to DocumentRelation types
RELATION_TYPE_MAP = {
    "duplicate": DocumentRelationChoices.DUPLICATE,
    "relates_to": DocumentRelationChoices.RELATES_TO,
    "blocked_by": DocumentRelationChoices.BLOCKED_BY,
    "blocking": DocumentRelationChoices.BLOCKED_BY,  # Will be reversed
}


class Command(BaseCommand):
    help = "Migrate Issues to Documents"
    ...
    def migrate_workspace(self, workspace, dry_run, batch_size):
        ...
        migrated_issue_ids = set(
            IssueToDocumentMapping.objects.filter(workspace=workspace)
    ...
    def migrate_issue(self, issue, workspace):
        """Migrate a single issue to a Document."""
        ...
        doc = Document.objects.create(
            ...
            access=Document.PRIVATE_ACCESS,
            owned_by=owner,
        ...
        # Record the mapping
        IssueToDocumentMapping.objects.create(
            issue_id=issue.id,
            document_id=doc.id,
            workspace=workspace,
        )

        # Migrate labels (M2M through DocumentLabel)
        for issue_label in IssueLabel.objects.filter(issue=issue, deleted_at__isnull=True):
            DocumentLabel.objects.create(
                document=doc,
        ...
        # Migrate assignees (M2M through DocumentAssignee)
        ...
        for subscriber in IssueSubscriber.objects.filter(issue=issue):
            DocumentSubscriber.objects.create(
                workspace=workspace,
                document=doc,
        ...
        for mention in IssueMention.objects.filter(issue=issue):
            DocumentMention.objects.create(
                workspace=workspace,
                document=doc,
        ...
        # Migrate issue properties as DocumentPropertyValues
        self.migrate_issue_properties(issue, doc, workspace)

        return doc

    def migrate_issue_properties(self, issue, doc, workspace):
...
class MigrateRelationsCommand(BaseCommand):
    ...
    """
    help = "Migrate IssueRelations to DocumentRelations (run after migrate_issues_to_documents)"
    ...
    def migrate_workspace_relations(self, workspace, dry_run):
        ...
        # Build issue->document mapping
        mappings = {
            m.issue_id: m.document_id
            for m in IssueToDocumentMapping.objects.filter(workspace=workspace)
        }
        ...
        for relation in relations:
            ...
            document_id = mappings[relation.issue_id]
            related_document_id = mappings[relation.related_issue_id]

            # Map relation type
            relation_type = RELATION_TYPE_MAP.get(
                relation.relation, DocumentRelationChoices.RELATES_TO
            )

            if not dry_run:
                DocumentRelation.objects.get_or_create(
                    workspace=workspace,
